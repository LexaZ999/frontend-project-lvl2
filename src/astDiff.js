...
const getAstDiff = (obj1, obj2, depth = 0, substatus) => {
  ...
  const result = uniqKeys.flatMap((key) => {
    ...
    const createDiffCell = (value1, value2, children) => {
      const diffCell = {
        key, value1, value2, depth, status, substatus, children,
      };
      return diffCell;
    };
    ...
    if (typeof val1 === 'object' && typeof val2 === 'object') {
      status = 'nested';
      return createDiffCell('nested', 'nested', getAstDiff(val1, val2, depth + 1));
    }
    if (typeof val1 === 'object' && typeof val2 !== 'object') {
      status = val2 ? 'nested changed to value' : 'deleted nested';
      return createDiffCell('nested', val2, getAstDiff(val1, {}, depth + 1, 'del'));
    }
    if (!_.has(obj1, key) && _.has(obj2, key) && typeof val2 === 'object') {
      status = 'added nested';
      return createDiffCell(val1, 'nested', getAstDiff({}, val2, depth + 1, 'add'));
    }
